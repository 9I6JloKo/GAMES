...
public class guiApp extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        enter.getButton().addActionListener((ActionEvent evt) -> {
            infoFirst.getInfo().setForeground(Color.BLUE);
            try{
                if( !loginfirst.getEditor().getText().isEmpty() &&
                    !passwordfirst.getEditor().getText().isEmpty()){
                    if(managers.isEmpty()){
                        Manager manager = new Manager();
                        manager.setName("Максон");
                        manager.setSurname("Грифон");
                        manager.setLogin("maksim");
                        manager.setPassword("12345");
                        manager.setPhone("53883833");
                    }
                    for(Manager manager : managers){
                        if(manager.getLogin() == loginfirst.getEditor().getText() &&
                           manager.getPassword() == passwordfirst.getEditor().getText()){
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        managerTabbed.addTab("Регистрация менеджера", enterPanel);
